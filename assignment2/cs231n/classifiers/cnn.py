...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache6 = affine_forward(x5, W3, b3)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss += 0.5 * self.reg * np.sum(W3 **2) + 0.5 * self.reg * (np.sum(W1 **2) + np.sum(W3 **2) + np.sum(W2 **2))
        dx6, dW3, db3 = affine_backward(dx, cache6)
        grads['W3'] = dW3 + self.reg * W3
        grads['b3'] = db3
        dx5 = relu_backward(dx6, cache5)
        dx4, dW2, db2 = affine_backward(dx5, cache4)
        grads['W2'] = dW2 + self.reg * W2
        grads['b2'] = db2
        dx3 = max_pool_backward_fast(dx4, cache3)
        dx2 = relu_backward(dx3, cache2)
        dx1, dW1, db1 = conv_backward_fast(dx2, cache1)
